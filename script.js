...
function guardarCalendario() {
    localStorage.setItem("datosCalendario", JSON.stringify(datosCalendario));
    localStorage.setItem("diasTachados", JSON.stringify(diasTachados));
}

// Cargar los datos guardados //
let datosGuardados = localStorage.getItem("datosCalendario");

if (datosGuardados) {
    try {
        datosCalendario = JSON.parse(datosGuardados);
    } catch (error) {
...
let diasTachadosGuardados = localStorage.getItem("diasTachados");

if (diasTachadosGuardados) {
    diasTachados = JSON.parse(diasTachadosGuardados);
}

// Obtener los datos del día seleccionado //
function obtenerDatosDia(dia) {

    let clave = añoMostrado + "-" + mesMostrado + "-" + dia;
...
function mostrarElementosDia() {
    ...
    let datosDia = obtenerDatosDia(diaSeleccionado);

    if (datosDia.length === 0) {

        listaDia.textContent = "No hay nada apuntado para este día.";
        ...
        for (let i = 0; i < datosDia.length; i++) {
			...
			elemento.textContent = datosDia[i].texto;

			if (datosDia[i].horaInicio && datosDia[i].horaFin) {
				elemento.textContent += " (" + datosDia[i].horaInicio + " - " + datosDia[i].horaFin + ")";
			} else if (datosDia[i].horaInicio) {
				elemento.textContent += " (desde " + datosDia[i].horaInicio + ")";
			} else if (datosDia[i].horaFin) {
				elemento.textContent += " (hasta " + datosDia[i].horaFin + ")";
			}
			
			elemento.style.backgroundColor = datosDia[i].color + "80";
			...
			botonEditar.onclick = function() {
				...
				txtElementoDia.value = datosDia[i].texto;
				colorElementoDia.value = datosDia[i].color;
				horaInicioElemento.value = datosDia[i].horaInicio;
				horaFinElemento.value = datosDia[i].horaFin;

				formularioDia.style.display = "block";
			};
			...
			botonEliminar.onclick = function() {
				let confirmar = confirm("¿Estás segura de que quieres eliminar este elemento?");
				if (confirmar) {
					datosDia.splice(i, 1);
					
					// Guardar el calendario después de eliminar //
					guardarCalendario();
...
for (let i = 0; i < nombresDias.length; i++) {
    ...
    diaSemana.textContent = nombresDias[i];
...
const ventanaDia = document.getElementById("ventanaDia");
console.log("Ventana del día:", ventanaDia);
...
const btnAñadirDia = document.getElementById("btnAñadirDia");

// Formulario para añadir un elemento //
const formularioDia = document.getElementById("formularioDia");

// Campo de texto del formulario //
const txtElementoDia = document.getElementById("txtElementoDia");

// Selector del color del elemento //
const colorElementoDia = document.getElementById("colorElementoDia");

// Hora de inicio del elemento //
const horaInicioElemento = document.getElementById("horaInicioElemento");

// Hora de fin del elemento //
const horaFinElemento = document.getElementById("horaFinElemento");
...
btnEmoji.onclick = function() {
    ...
    if (menuEmojis.style.display === "block") {
...
const botonesEmoji = menuEmojis.querySelectorAll("button");

for (let i = 0; i < botonesEmoji.length; i++) {

    botonesEmoji[i].onclick = function() {

        txtElementoDia.value = botonesEmoji[i].textContent + " " + txtElementoDia.value;
...
const btnGuardarElemento = document.getElementById("btnGuardarElemento");
...
btnCerrarDia.onclick = function() {

    ventanaDia.style.display = "none";

};

// Botón Cancelar cierra el formulario //
btnCancelarElemento.onclick = function() {

    formularioDia.style.display = "none";

};

// Botón Guardar //
btnGuardarElemento.onclick = function() {

    // Obtener los datos del formulario //
    let textoElemento = txtElementoDia.value;
    let colorElemento = colorElementoDia.value;
    ...
    let clave = añoMostrado + "-" + mesMostrado + "-" + diaSeleccionado;

    // Si el día todavía no tiene elementos, crear una lista //
    if (!datosCalendario[clave]) {
	...
	if (elementoEditando === null) {
		...
		datosCalendario[clave].push(elemento);
	} else {
		// Si estamos editando, sustituir el elemento existente //
		datosCalendario[clave][elementoEditando] = elemento;
	}
	
	// Guardar el calendario //
	guardarCalendario();
	
	// Ya no estamos editando ningún elemento //
	elementoEditando = null;
	
	// Actualizar la lista del día //
	mostrarElementosDia();

	// Actualizar las casillas del calendario //
	mostrarCalendario();

	console.log("Elemento guardado:", elemento);

    // Limpiar el formulario //
    txtElementoDia.value = "";
    colorElementoDia.value = "#000000";
...
function mostrarCalendario() {
    ...
    mesActual.textContent = nombresMeses[mesMostrado] + " " + añoMostrado;

    // Borrar los días que estaban mostrados //
    diasCalendario.innerHTML = "";

    // Día de la semana en el que empieza el mes //
    let primerDia = new Date(añoMostrado, mesMostrado, 1).getDay();

    // Convertir domingo = 0 en domingo = 6 y lunes = 0 //
    primerDia = (primerDia + 6) % 7;

    // Número de días que tiene el mes //
    let numeroDias = new Date(añoMostrado, mesMostrado + 1, 0).getDate();
    ...
    for (let i = 0; i < primerDia; i++) {
        ...
        diasCalendario.appendChild(espacioVacio);
    }

    // Crear las casillas de los días //
    for (let dia = 1; dia <= numeroDias; dia++) {

		let casillaDia = document.createElement("div");

		// Mostrar el número del día //
		let numeroDia = document.createElement("div");
		numeroDia.textContent = dia;
		
		numeroDia.style.display = "inline-block";
		casillaDia.appendChild(numeroDia);

		// Crear la X para tachar el día //
		let botonTachar = document.createElement("button");
		botonTachar.textContent = "X";
		
		botonTachar.onclick = function(event) {
			event.stopPropagation();

			let claveDia = añoMostrado + "-" + mesMostrado + "-" + dia;

			if (diasTachados[claveDia]) {

				diasTachados[claveDia] = false;

				casillaDia.style.backgroundImage = "";
			} else {

				diasTachados[claveDia] = true;

				casillaDia.style.backgroundImage = "linear-gradient(135deg, transparent 49%, red 49%, red 51%, transparent 51%)";
			}

			guardarCalendario();
		};
		
		botonTachar.style.fontSize = "10px";
		botonTachar.style.padding = "0";
		botonTachar.style.width = "18px";
		botonTachar.style.height = "18px";

		botonTachar.style.display = "inline-block";
		casillaDia.appendChild(botonTachar);
		
		// Comprobar si el día estaba tachado //
		let claveDia = añoMostrado + "-" + mesMostrado + "-" + dia;

		if (diasTachados[claveDia]) {
			casillaDia.style.backgroundImage = "linear-gradient(135deg, transparent 48%, black 48%, black 52%, transparent 52%)";
		}

		// Obtener los elementos guardados de este día //
		let clave = añoMostrado + "-" + mesMostrado + "-" + dia;
		let elementosDia = datosCalendario[clave];
		...
		if (elementosDia) {
			...
			for (let i = 0; i < elementosDia.length; i++) {
				...
				resumenElemento.textContent = elementosDia[i].texto;

				if (elementosDia[i].horaInicio && elementosDia[i].horaFin) {
					resumenElemento.textContent += " (" + elementosDia[i].horaInicio + " - " + elementosDia[i].horaFin + ")";
				} else if (elementosDia[i].horaInicio) {
					resumenElemento.textContent += " (desde " + elementosDia[i].horaInicio + ")";
				} else if (elementosDia[i].horaFin) {
					resumenElemento.textContent += " (hasta " + elementosDia[i].horaFin + ")";
				}

				// Hacer la letra más pequeña //
				resumenElemento.style.fontSize = "9px";

				// Poner el mismo color de fondo que en la lista //
				resumenElemento.style.backgroundColor = elementosDia[i].color + "80";

				casillaDia.appendChild(resumenElemento);
			}
		}

		// Hacer que la casilla se pueda pulsar //
		casillaDia.onclick = function() {

			diaSeleccionado = dia;
			console.log("Has pulsado el día " + diaSeleccionado);
			console.log("Intentando abrir la ventana");

			// Mostrar la fecha seleccionada //
			tituloDia.textContent = diaSeleccionado + " de " + nombresMeses[mesMostrado] + " de " + añoMostrado;

			mostrarElementosDia();

			ventanaDia.style.display = "block";
		};

		diasCalendario.appendChild(casillaDia);
	}
}
...
btnMesSiguiente.onclick = function() {

    mesMostrado++;

    // Si pasamos de diciembre, volvemos a enero y aumentamos el año //
    if (mesMostrado > 11) {
        mesMostrado = 0;
        añoMostrado++;
    }
...
btnMesAnterior.onclick = function() {
    ...
    // Si pasamos de enero, volvemos a diciembre y restamos un año //
    if (mesMostrado < 0) {
        mesMostrado = 11;
        añoMostrado--;
    }
...
const btnInicioCalendario = document.getElementById("btnInicioCalendario");

// Botón de configuración //
const btnConfiguracionInicio = document.getElementById("btnConfiguracionInicio");
const btnInicioConfiguracion = document.getElementById("btnInicioConfiguracion");
...
btnInicioConfiguracion.onclick = function() {
...
botonCalendario.onclick = function() {
...
// BOTÓN PARA VOLVER AL INICIO DESDE EL CALENDARIO //

btnInicioCalendario.onclick = function() {
...
let datosAsignaturasGuardados = localStorage.getItem("asignaturasGuardadas");

if (datosAsignaturasGuardados !== null) {
    asignaturasGuardadas = JSON.parse(datosAsignaturasGuardados);
}
